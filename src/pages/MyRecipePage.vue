<template>
  <div class="container" >
    <h1 class="title" style="text-align: center">My Personal Recipes</h1>
    <div class="mt-3">
      <b-container>
    ...
    async SearchRecipes() {
      try {
          ...
          const response = await this.axios.get(
            this.$root.store.server_domain + "/users/GetPreviewPersonalRecipes",
            {withCredentials:true}
          );
          console.log(response);
          const recipes = response.data.preview_personal_recipes;
          this.recipes = [];
